...
public class RetrieveList {
	...
	public void retrieveListOfAddressBookSubscriptionsUser1(){
		...
		Response response = RestAssured.expect().log().ifError().statusCode(200).body(
				"abChangesSubscriptionCollection.resourceURL", StringContains.containsString(cleanUserID),
				"abChangesSubscriptionCollection.abChangesSubscription.size()", Matchers.is(1),
				"abChangesSubscriptionCollection.abChangesSubscription[0].resourceURL", StringContains.containsString(cleanUserID),
				"abChangesSubscriptionCollection.abChangesSubscription[0].callbackReference.notifyURL", IsEqual.equalTo(callbackURL[i]),
