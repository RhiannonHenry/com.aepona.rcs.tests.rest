...
public class RetrieveList {
	...
	public void listUser1Subscriptions(){
		...
		Response response = RestAssured.expect().log().ifError().statusCode(200).body(
				"sessionSubscriptionList.resourceURL", StringContains.containsString(cleanUserID),
				"sessionSubscriptionList.sessionSubscription.size()", Matchers.is(1),
				"sessionSubscriptionList.sessionSubscription[0].callbackReference.callbackData", IsEqual.equalTo(userID),  
				"sessionSubscriptionList.sessionSubscription[0].callbackReference.notifyURL", IsEqual.equalTo(callbackURL[1]),
	...
	public void listUser2Subscriptions(){
		...
		Response response = RestAssured.expect().log().ifError().statusCode(200).body(
				...
				"sessionSubscriptionList.sessionSubscription.size()", Matchers.is(1),
				"sessionSubscriptionList.sessionSubscription[0].callbackReference.callbackData", IsEqual.equalTo(userID),  
				"sessionSubscriptionList.sessionSubscription[0].callbackReference.notifyURL", IsEqual.equalTo(callbackURL[i]),
				"sessionSubscriptionList.sessionSubscription[0].callbackReference.notificationFormat", IsEqual.equalTo("JSON")
				).get(url);
	...
	public void listUserWithNoSubscriptions(){
		String userID = "+15554000004";
		int i = 2;
		
		startNotificationChannel(user1, i);
		//subscribeToSessionNotifications(userID, i);
		...
		RestAssured.authentication = RestAssured.basic(user1,
				applicationPassword);
