...
private final Logger LOGGER = LoggerFactory.getLogger(Subscribe.class);
	...
	public void subscribeFileTransfer(){
		...
		String requestData = requestDataClean(fileTransferRequestData, userID, CALLBACK_URL[i]);
		...
		RestAssured.authentication = RestAssured.digest(userID,applicationPassword);
		Response response = RestAssured.given().contentType("application/json").body(requestData).expect().statusCode(201).post(url);
		
		FILE_TRANSFER_SUB_URL[i] = response.jsonPath().getString("fileTransferSubscription.resourceURL");
		LOGGER.info("Response Received = "+response.getStatusCode());
		LOGGER.info("File Transfer Subscription URL = "+FILE_TRANSFER_SUB_URL[i]);
		endTest(test);
	}
	...
	public void subscribeFileTransfer3(){
		...
		String requestData = requestDataClean(fileTransferRequestData, userID, CALLBACK_URL[1]);
		String url = replace(fileTransferSubscriptionURL, apiVersion, userID);
		LOGGER.info("Request Body = "+requestData);
		LOGGER.info("URL = "+baseURI+url);
		
		RestAssured.authentication = RestAssured.digest(userID,applicationPassword);
		Response response = RestAssured.given().contentType("application/json").body(requestData).expect().statusCode(401).post(url);
	...
	public void subscribeFileTransfer4(){
		...
		RestAssured.authentication = RestAssured.digest(user1,applicationPassword);
		Response response = RestAssured.given().contentType("application/json").body(requestData).expect().statusCode(400).post(url);
	...
	private String requestDataClean(String fileTransferRequestData,
		...
		String clean = fileTransferRequestData.replace("{CALLBACK}", callback).replace(
				"{USERID}", userID);
		return clean;
	}
