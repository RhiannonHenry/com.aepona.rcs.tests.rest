...
public class RetrieveIndividual {
	...
	public void retrieveIndividualSubscription(){
		...
		Response response = RestAssured.expect().log().ifError().statusCode(200).body(
				"fileTransferSubscription.resourceURL", StringContains.containsString(cleanUserID),
				"fileTransferSubscription.callbackReference.callbackData", IsEqual.equalTo(userID), 				
				"fileTransferSubscription.callbackReference.notifyURL", IsEqual.equalTo(CALLBACK_URL[i]),
